...
const TITLE = "ElevenLabs: AI Voice and Text-to-Speech for Creators, Podcasters, and Developers"
...
const { data: created, error } = await supabase
  ...
  .insert({
    ...
    meta_description: "ElevenLabs — AI voice and text-to-speech platform for creators, podcasters, audiobook producers, and developers.".slice(0, 155),
    ...
    approval_notes: "Operator-corrected draft. Status: draft (NOT published). Real affiliate link inserted.",
  })
  .select("id")
  .single()

if (error) { console.error("Failed to save:", error.message); process.exit(1) }

console.log(`CREATED corrected ElevenLabs draft (${created.id})`)
console.log(`  Title: ${TITLE}`)
console.log(`  Body: ${BODY.length} chars`)
console.log(`  Affiliate link: ${AFFILIATE_URL}`)
console.log(`  Status: draft — ready for approval`)
