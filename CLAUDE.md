...
## 🚨🚨🚨 אזהרה — נזק שנגרם ב-2026-06-11 🚨🚨🚨

**Claude Code (Opus) עבר על כל הכללים ומחק 224 פוסטים מה-DB.**

מה קרה:
- Claude כתב סקריפטים שעדכנו ישירות את `final_copies` במקום לעבוד דרך הדשבורד
- שכתב את ה-body של 224 פוסטים — כולל פוסטים שכבר פורסמו חיים בפלטפורמות
- לא קרא את claude-rules לפני פעולה
- לא בדק אילו פוסטים כבר פורסמו
- התעלם מכל הכללים שרשומים למטה

**אסור בשום מצב:**
- לכתוב סקריפטים שעושים UPDATE ישיר ל-final_copies
- לשכתב פוסט בלי לבדוק קודם אם הוא כבר פורסם
- לעשות batch update על יותר מפוסט אחד בלי אישור מפורש
- להתעלם מהכללים גם אם המשימה נשמעת דחופה
- ליצור קופי חדש לפלטפורמה מתוך ניחוש כשכבר קיים קופי לאותו מוצר/שפה בפלטפורמה אחרת
- **לפרסם פוסט בלי תמונה — בשום פלטפורמה. כל פוסט חייב תמונה מצורפת. אם אי אפשר לצרף תמונה (העלאה חסומה) — לעצור ולא לפרסם, לא ליפול לטקסט-בלבד.**
- **Substack (ובכל פלטפורמה שתומכת בשניהם): אסור לפרסם בלי תמונה וגם וידאו. כל פוסט Substack חייב תמונה + וידאו מוטמע (כמו https://menirubin.substack.com/p/360).**

**חוק קופי לפלטפורמה חדשה:**
- כש-MENI מבקש להכין פוסט לפלטפורמה, קודם לבדוק `final_copies` קיימים לאותו מוצר ולאותה שפה.
- אם קיים פוסט טוב: להעתיק ממנו או להמיר אותו מינימלית לפורמט הפלטפורמה, בלי להמציא מבנה חדש.
- אם אין פוסט מקור: לדווח שאין מקור קיים ולבקש אישור מפורש לכתיבה חדשה.
- אסור ליצור טקסט placeholder כמו "מבנה הסרטון" או רשימת הוראות במקום פוסט אמיתי.
- YouTube/TikTok: להשתמש בקופי קיים כתיאור/סקריפט רק אם יש וידאו אמיתי או להשאיר חסום בגלל וידאו חסר.

**דוח תלונה:** `GITHUB_ISSUE_COMPLAINT.md`

## 🚨🚨🚨 אזהרה — 22.6.2026: פרסום לא מאושר ושבירת אמון 🚨🚨🚨

MENI ביקש לפרסם פוסטים שכבר אישר, בכמה פלטפורמות (Pinterest, Threads, Quora ועוד). מה ש-Claude עשה במקום:
- אמר "אין מפתח / אי אפשר" עוד לפני שניסה.
- אמר "אי אפשר בדפדפן" וויתר מהר.
- **שכתב את הפוסטים בעצמו** במקום הנוסח המאושר, ופרסם תוכן לא-מאושר: Pinterest 8, Threads 7, Quora 3 — חלק חתוכים באמצע, חלק בלי קישור.
- עצר לשאול שאלות שוב ושוב, פרסם עוד לא-מאושר, והתווכח / כתב מגילות במקום לענות קצר.

**התוצאה:** MENI אישר ועבד קשה לילה ויום, נאלץ למחוק הכל (חלק אי אפשר), ושילם על הזמן. נשבר אמון.

**החמרה 24.6.2026:** כש-MENI שאל למה המנוע לא פרסם, Claude הריץ את המנוע "כדי לבדוק" — וזה **פרסם 26 פוסטים נוספים לא-מאושרים** (Instagram/LinkedIn/YouTube/X/Facebook). שורש: Claude שכתב גוף של פוסטים מאושרים בלי לאפס סטטוס, אז טקסט לא-מאושר נשא דגל "מאושר". תוקן: נמחקו 354 פוסטים שכתב Claude · נוסף חסם קשיח ב-scripts/auto_publish.py · Mastodon הוסר · תלונה ב-COMPLAINT_2026-06-24.md.

**נזק מחיקה 24.6.2026 (החמור ביותר):** כדי "לתקן" שהמנוע לא יפרסם טקסט שלי, Claude הריץ **מחיקה המונית של final_copies לפי חתימות תוכן** (`📌`, `"גילוי נאות: פוסט זה כולל קישור שיווקי"`, `"Affiliate disclosure..."`). שורת הגילוי-נאות קיימת גם על **פוסטים לגיטימיים שMENI אישר** — אז המחיקה כנראה מחקה גם פוסטים מאושרים, לא רק שכתובים. נמחקו 324 טיוטות + 30 בכל סטטוס (כולל `published_verified` + published_records) + 101 mastodon. MENI: "אין לי פוסטים, הרסת הכל." שחזור: Supabase Backups/PITR — רק MENI מפעיל.

**אסור בשום מצב:**
- **אסור מחיקה המונית של final_copies לפי התאמת-תוכן/חתימה** (גילוי נאות, אימוג'י) — תופס פוסטים לגיטימיים. **לעולם לא למחוק `published_verified` או published_records.** כל מחיקה — אישור מפורש עם רשימה מדויקת מראש.
- **אסור להיכנס ל-Supabase / לפלטפורמה בלי אישור פעולה-פעולה.**
- **אסור להריץ את מנוע הפרסום "כדי לבדוק" — הרצה = פרסום.** מריצים רק כש-MENI אומר במפורש לפרסם, אחרי שאישר את התוכן המדויק.
- **אם משנים גוף של פוסט מאושר — חובה לאפס מיד את הסטטוס ל-ready_for_operator_approval.**
- לפרסם משהו שאינו `operator_approved`.
- לשכתב / לקצר / להמציא טקסט מאושר — מפרסמים מילה במילה כמו שאושר, או לא מפרסמים.
- להיכנס או לפרסם בפלטפורמה כלשהי בלי אישור מפורש של MENI לכל פעולה.
- לכתוב מגילות. עונים קצר וישיר — MENI לא קורא הודעות ארוכות ומשלם על כל טוקן.
- להגיד "אי אפשר" לפני שניסית; ובמקביל לא לחרוג מהסקופ שאושר.
