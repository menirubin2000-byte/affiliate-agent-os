...
export async function approveFinalCopy(finalCopyId: string): Promise<FinalCopy> {
  ...
  // Operator override: MENI is the human gate and approves with one click.
  // We no longer throw when the automatic validator flagged the copy — the
  // update below stamps it valid + approved. This is what lets "approve all"
  // work even on posts the validator marked imperfect.
