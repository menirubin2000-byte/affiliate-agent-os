...
import { countPendingApprovalItems } from "@/lib/db"

export default async function DashboardLayout({ children }: { children: ReactNode }) {
  let pendingApprovalCount = 0
  try {
    pendingApprovalCount = await countPendingApprovalItems()
  } catch {
    // Silently default to 0 if DB is unavailable
  }
    ...
    <div className="min-h-screen bg-muted/30">
      <div className="mx-auto grid min-h-screen max-w-[1600px] lg:grid-cols-[260px_minmax(0,1fr)]">
        <div className="border-b border-border/70 lg:border-b-0">
          <DashboardSidebar pendingApprovalCount={pendingApprovalCount} />
        </div>
