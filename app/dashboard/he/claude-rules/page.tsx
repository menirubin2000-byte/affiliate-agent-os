...
const FORBID_LIST = [
  `🚨 22.6.2026: אסור לפרסם משהו שאינו operator_approved. אסור לשכתב / לקצר / להמציא טקסט מאושר — מפרסמים מילה במילה כמו שאושר, או לא מפרסמים. (באותו יום Claude שכתב פוסטים בעצמו ופרסם 18 פוסטים לא-מאושרים, חתוכים ובלי קישור — ושבר אמון.)`,
  `🚨 24.6.2026: אסור להריץ את מנוע הפרסום "כדי לבדוק" — הרצה = פרסום (באותו יום הרצה כזו פרסמה 26 פוסטים נוספים לא-מאושרים). אם משנים גוף של פוסט מאושר — חובה לאפס מיד את הסטטוס ל-ready_for_operator_approval.`,
  `🚨 24.6.2026 (החמור ביותר): אסור מחיקה המונית של final_copies לפי חתימת-תוכן (אימוג'י 📌, שורת "גילוי נאות") — השורות האלה קיימות גם על פוסטים לגיטימיים שMENI אישר, ומחיקה כזו מחקה גם אותם. באותו יום נמחקו 324 טיוטות + 30 בכל סטטוס (כולל published_verified + published_records). לעולם לא למחוק published_verified או published_records. כל מחיקה — רק עם אישור מפורש והרשימה המדויקת מוצגת מראש. אסור להיכנס ל-Supabase בלי אישור פעולה-פעולה.`,
  `🚨 אסור להיכנס או לפרסם בפלטפורמה כלשהי בלי אישור מפורש של MENI לכל פעולה.`,
  `🚨 אסור לכתוב מגילות. עונים קצר וישיר — MENI לא קורא הודעות ארוכות ומשלם על כל טוקן. אסור להתווכח במקום לענות.`,
...
const FORBIDDEN = [
  "פרסום פוסט בלי תמונה — בשום פלטפורמה. כל פוסט חייב תמונה מצורפת. אם אי אפשר להעלות תמונה (העלאה חסומה) — לעצור ולא לפרסם, לא ליפול לטקסט-בלבד.",
  "Substack (ובכל פלטפורמה שתומכת בשניהם): פרסום בלי תמונה וגם וידאו. כל פוסט Substack חייב תמונה + וידאו מוטמע.",
...
const APPROVAL_UI_RULES = [
  "דף האישור מחולק ל-2 מסכים בלבד: תוכנות ומוצרים.",
  "דפי הסקירות הציבוריים (/software ו-/products) אינם דפי אישור.",
  "בכל מוצר או תוכנה רואים את כל 13 הפלטפורמות הקיימות במערכת. אסור להמציא פלטפורמות חדשות.",
  "מסמנים רק את הפלטפורמות שרוצים לערוך או לאשר.",
  "מתחת לכל פוסט מוצגת מגבלת התווים של אותה פלטפורמה.",
  "שדה העריכה מעדכן רק את הפלטפורמות שסומנו.",
  "כפתור האישור מאשר רק את הפלטפורמות שסומנו ורק אם הן במצב ready_for_operator_approval.",
]
const PROOF_LINE =
...
export default function ClaudeRulesPage() {
  return (
    <div dir="rtl" className="space-y-6 text-right">
      ...
      <Card className="border-primary/30 bg-primary/5">
        <CardHeader>
          <CardTitle>כללי אישור קבועים במערכת</CardTitle>
          <CardDescription>
            אלה חוקים פנימיים לקלוד. לא משנים את זרימת האישור בלי הוראה מפורשת מ-MENI.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <ul className="list-inside list-disc space-y-1 text-sm">
            {APPROVAL_UI_RULES.map((line, i) => (
              <li key={i}>{line}</li>
            ))}
          </ul>
        </CardContent>
